...
> con adaptaciones para el sitio de estudio. Este archivo fue movido a la carpeta `doc/`
> como parte de la reorganización del repositorio.

### Train a new CoastSat classifier

CoastSat's shoreline mapping algorithm uses an image classification scheme to label each pixel into 4 classes: sand, water, white-water and other land features. While this classifier has been trained using a wide range of different beaches, it may be that it does not perform very well at specific sites that it has never seen before.
